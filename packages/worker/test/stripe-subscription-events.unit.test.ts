...
import { buildSubscriptionEventMetadata, extractStripeSubscriptionData, getPlanChangeTrackingEventName, getSubscriptionTrackingState } from '../src/stripe'
...
describe('[Capgo parity] stripe subscription event classification', () => {
  it.concurrent('marks monthly to yearly cadence changes as upgraded without a plan change', () => {
    const stripeData = extractStripeSubscriptionData({
      data: {
        object: {
    ...
    expect(getSubscriptionTrackingState(stripeData, 'updated')).toEqual({
      shouldSendPlanChange: false,
      statusName: 'upgraded',
    })
    expect(getPlanChangeTrackingEventName('upgraded')).toBe('User Upgraded')
    expect(
  ...
  it.concurrent('keeps same-cadence plan switches as plan changes instead of upgrades', () => {
    ...
    expect(getPlanChangeTrackingEventName('updated')).toBe('User Plan Changed')
    expect(
      buildSubscriptionEventMetadata(
        stripeData,
        makePlan({
