...
import { isUpdateEnumerationLimited, recordUpdateEnumerationMiss, updateEnumerationLimitedResponse } from '../src/update-oracle-guard'
...
describe('[Capgo parity] update enumeration guard', () => {
  ...
  it('does not block known apps behind the same IP before they are validated as misses', async () => {
    ...
    await app.fetch(new Request('http://localhost/updates', { method: 'POST', headers, body: JSON.stringify({ app_id: 'com.missing.first' }) }))
    await app.fetch(new Request('http://localhost/updates', { method: 'POST', headers, body: JSON.stringify({ app_id: 'com.missing.second' }) }))

    const known = await app.fetch(new Request('http://localhost/updates', { method: 'POST', headers, body: JSON.stringify({ app_id: 'com.real.app' }) }))
  ...
  it('short-circuits already limited IPs before recording another app ID', async () => {
    ...
    const blocked = await app.fetch(new Request('http://localhost/limited', { method: 'POST', headers }))
    expect(blocked.status).toBe(429)
    await expect(blocked.json()).resolves.toMatchObject({ error: 'on_premise_app' })
  })
