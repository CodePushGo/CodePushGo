...
import { runSupabaseResultWithRetry, retryWithBackoff } from '../src/retry'
import { fetchUploadHandlerWithRetry, X_UPLOAD_HANDLER_RETRYABLE } from '../src/upload-retry'
import { existInEnv, getEnv, shouldRetryManifestSizeLookup } from '../src/utils'

describe('[Capgo parity] backend alert resilience helpers', () => {
  ...
  it('retries retryable durable object resets for replayable upload requests', async () => {
    ...
    const response = await fetchUploadHandlerWithRetry(
      handler,
  ...
  it('retries transient PostgREST 502 responses for cron_stat_app', async () => {
    let attempts = 0
    const result = await runSupabaseResultWithRetry(
  ...
  it.concurrent('retries transient PostgREST statuses for on_manifest_create updates', async () => {
    let attempts = 0
    await runSupabaseResultWithRetry(
      'test_manifest_update_retry',
      async () => {
        attempts += 1
  ...
  it.concurrent('marks missing manifest storage size as queue-retryable unless a trusted size already exists', async () => {
    expect(shouldRetryManifestSizeLookup(0, 0)).toBe(true)
    expect(shouldRetryManifestSizeLookup(0, null)).toBe(true)
    expect(shouldRetryManifestSizeLookup(0, 128)).toBe(false)
    expect(shouldRetryManifestSizeLookup(128, 0)).toBe(false)
  })
