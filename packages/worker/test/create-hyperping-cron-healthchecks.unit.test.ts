...
import { buildHealthcheckPayload, type CronTaskRow } from '../../../scripts/create_hyperping_cron_healthchecks'
...
describe('[Capgo parity] create_hyperping_cron_healthchecks', () => {
  it.concurrent('uses second-based periods and floors grace to one minute', () => {
    const candidate = buildHealthcheckPayload(createCronTaskRow({ second_interval: 10 }), 10 * 60, 'UTC')
  ...
  it.concurrent('uses one-minute grace for one-minute cron tasks', () => {
    const candidate = buildHealthcheckPayload(createCronTaskRow({ minute_interval: 1 }), 10 * 60, 'UTC')
  ...
  it.concurrent('keeps long-schedule grace at the configured maximum', () => {
    const candidate = buildHealthcheckPayload(createCronTaskRow({ hour_interval: 2 }), 10 * 60, 'UTC')
  ...
  it.concurrent('rounds grace down so it does not exceed the configured maximum', () => {
    const candidate = buildHealthcheckPayload(createCronTaskRow({ hour_interval: 2 }), 61, 'UTC')
