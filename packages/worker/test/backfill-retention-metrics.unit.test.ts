import type { RevenuePlanRow, StripeEventLike, StripeSubscriptionItemLike } from '../../../scripts/backfill_retention_metrics'
import { describe, expect, it } from 'vitest'
import {
  aggregateRevenueMovementEvents,
  buildRevenueMovementEvents,
  fetchStripeEvents,
  findMissingResetSnapshotEventIds,
  getDatabaseUrl,
  getRequiredDatabaseUrl,
  mergeMetricRows,
  shouldAllowSelfSignedPgCertificate,
  summarizeDailyRevenueMetrics,
} from '../../../scripts/backfill_retention_metrics'
...
] satisfies RevenuePlanRow[]

function subscriptionItem(priceId: string, productId: string, currentPeriodEnd?: number, usageType = 'licensed'): StripeSubscriptionItemLike {
  return {
...
function subscriptionEvent(
  ...
  const previousAttributes: StripeEventLike['data']['previous_attributes'] = {}
  ...
  if (previous && 'status' in previous)
    previousAttributes.status = previous.status
  ...
  return {
    ...
    data: {
      ...
      previous_attributes: previous ? previousAttributes : undefined,
    },
  }
}

describe('[Capgo parity] retention metric backfill helpers', () => {
