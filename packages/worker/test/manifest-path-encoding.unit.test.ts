...
import { getManifestStorageCandidateKeys, normalizeLegacyEncodedManifestFileName } from '../src/manifest-encoding'
import { getManifestUrl } from '../src/manifest-url'
...
describe('[Capgo parity] manifest path encoding', () => {
  it.concurrent.each([
    ...
    const manifest = getManifestUrl('https://example.com/updates', 123, [{
      file_name: fileName,
      file_hash: 'hash',
      s3_path: s3Path,
    }], 'device-id')
