...
import { bundleUsageTestUtils } from '../src/bundle-usage'

describe('[Capgo parity] bundle usage helpers', () => {
  ...
  it('buildDailyReportedCountsByName aggregates daily get counts by version', () => {
    ...
    const counts = bundleUsageTestUtils.buildDailyReportedCountsByName(usage, dates, versions)
  ...
  it('fillMissingDailyCounts carries forward historical zero days', () => {
    ...
    const filled = bundleUsageTestUtils.fillMissingDailyCounts(counts, dates, versions, '2024-11-01')
  ...
  it('convertCountsToPercentagesByName converts daily counts into 0-100 share', () => {
    ...
    const percentages = bundleUsageTestUtils.convertCountsToPercentagesByName(counts, dates, versions)
  ...
  it('getLatestDayVersionShare returns top version share from latest day with data', () => {
    ...
    const latest = bundleUsageTestUtils.getLatestDayVersionShare(versions, dates, counts)
