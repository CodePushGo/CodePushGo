...
import { adminStatsBodySchema, buildPluginBreakdownResult, MAX_ADMIN_STATS_LIMIT, MAX_ADMIN_STATS_OFFSET, normalizeAnalyticsLimit } from '../src/admin-stats'

describe('[Capgo parity] admin stats validation', () => {
  const baseBody = {
  ...
  it.each([
    ...
    const parsed = adminStatsBodySchema.safeParse({ ...baseBody, ...body })
  ...
  it('accepts the trial plan breakdown metric', () => {
    const parsed = adminStatsBodySchema.safeParse({
      ...baseBody,
...
describe('[Capgo parity] normalizeAnalyticsLimit', () => {
  it.each([
...
describe('[Capgo parity] buildPluginBreakdownResult', () => {
  it('aggregates plugin versions with major breakdown and top app IDs', () => {
