...
import { fetchPublicUrl, getPublicHostnameValidationError, getPublicUrlSyntaxValidationError } from '../src/public-url'
...
describe('[Capgo parity] public outbound URL validation', () => {
  ...
  it('can treat missing DNS answers as either allowed or invalid', async () => {
    ...
    await expect(getPublicHostnameValidationError('https://unresolved.example/webhook', { messages, requireDnsResolution: false })).resolves.toBeNull()
    await expect(getPublicHostnameValidationError('https://unresolved.example/webhook', { messages, requireDnsResolution: true })).resolves.toBe('dns')
  })
  ...
  it('revalidates redirect targets before following them', async () => {
    ...
    const result = await fetchPublicUrl('https://safe.example/', undefined, { messages, requireDnsResolution: true })
    expect(result).toMatchObject({ error: 'public host', response: null, finalUrl: null })
