...
export function encodeManifestPathSegments(path: string) {
  return path.split('/').map(segment => encodeURIComponent(segment)).join('/')
}

export function decodeManifestPathSegments(path: string) {
  try {
...
export function normalizeLegacyEncodedManifestFileName(fileName: string | null | undefined, s3Path: string | null | undefined) {
  if (fileName == null)
    return null
...
export function getManifestStorageCandidateKeys(s3Path: string) {
