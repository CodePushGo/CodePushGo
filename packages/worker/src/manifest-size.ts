...
function versionIdFromDownloadUrl(downloadUrl: string | null | undefined) {
  if (!downloadUrl)
    return null
  try {
...
export function parseManifestSizeVersionId(value: unknown) {
...
export function buildManifestDownloadSizeResult(
  ...
  const resultFiles = files.map((file): ManifestSizeResultFile => {
    ...
    if (typeof size === 'number' && size > 0) {
      totalSize += size
      knownFiles += 1
      return { file_name: file.file_name, file_hash: file.file_hash, download_url: file.download_url, size }
    }

    unknownFiles += 1
    return { file_name: file.file_name, file_hash: file.file_hash, download_url: file.download_url, error: 'size_unknown' }
  })

  return { totalSize, knownFiles, unknownFiles, files: resultFiles }
}
