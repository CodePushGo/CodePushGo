...
import { comparePackages, summarizeCompatibility } from '../src/bundle-compatibility'
...
describe('comparePackages', () => {
  ...
  it.concurrent('marks a newly added package as added + incompatible', () => {
    const a = byName('a', comparePackages([pkg('a', '1.0.0')], []))
  ...
  it.concurrent('marks a removed package as removed + compatible', () => {
    const a = byName('a', comparePackages([], [pkg('a', '1.0.0')]))
  ...
  it.concurrent('flags a non-intersecting version change', () => {
    const a = byName('a', comparePackages([pkg('a', '2.0.0')], [pkg('a', '1.0.0')]))
    expect(a.status).toBe('changed')
    expect(a.compatible).toBe(false)
    expect(a.reasons).toEqual(['version_mismatch'])
  })

  it.concurrent('treats an intersecting version range as compatible', () => {
    const a = byName('a', comparePackages([pkg('a', '1.2.0')], [pkg('a', '^1.0.0')]))
    expect(a.reasons).not.toContain('version_mismatch')
    expect(a.compatible).toBe(true)
  })

  it.concurrent('flags changed iOS native checksum', () => {
    const a = byName('a', comparePackages(
  ...
  it.concurrent('flags changed Android native checksum', () => {
    const a = byName('a', comparePackages(
  ...
  it.concurrent('flags both platforms when both checksums changed', () => {
    ...
    expect(a.reasons).toEqual(['both_platforms_changed'])
  })
  ...
  it.concurrent('ignores checksum when only one side has it', () => {
    const a = byName('a', comparePackages([pkg('a', '1.0.0', { ios_checksum: 'i2' })], [pkg('a', '1.0.0')]))
    expect(a.compatible).toBe(true)
  })
