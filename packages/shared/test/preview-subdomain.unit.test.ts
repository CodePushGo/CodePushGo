...
import { buildChannelPreviewSubdomain, buildPreviewSubdomain, encodePreviewAppId, parsePreviewHostname } from '../src/preview-subdomain'

describe('[Capgo parity] preview subdomain encoding', () => {
  it.concurrent('round-trips app IDs with reserved hostname characters', () => {
    ...
    expect(parsePreviewHostname(`${subdomain}.preview.codepushgo.app`)).toEqual({ appId, versionId })
  })
  ...
  it.concurrent('keeps dotted and double-underscore app IDs in separate preview namespaces', () => {
    ...
    expect(parsePreviewHostname(`${dottedSubdomain}.preview.codepushgo.app`)).toEqual({ appId: dottedAppId, versionId: 77014920 })
    expect(parsePreviewHostname(`${underscoredSubdomain}.preview.codepushgo.app`)).toEqual({ appId: underscoredAppId, versionId: 77015428 })
  })

  it.concurrent('parses legacy preview hostnames for existing links', () => {
    expect(parsePreviewHostname('com__example__app-42.preview.codepushgo.app')).toEqual({
  ...
  it.concurrent('round-trips stable channel preview hostnames', () => {
    ...
    expect(subdomain).not.toMatch(/[.A-Z]/)
    expect(parsePreviewHostname(`${subdomain}.preview.codepushgo.app`)).toEqual({ appId, channelId })
  })
  ...
  it.concurrent('returns null for malformed hostnames', () => {
    ...
    expect(parsePreviewHostname('1-com-0example.preview.capgo.app')).toBeNull()
  })
  ...
  it.concurrent('does not mistake legacy double-hyphen app IDs for the new separator', () => {
    expect(parsePreviewHostname('com--example__app-42.preview.codepushgo.app')).toEqual({
      appId: 'com--example.app',
      versionId: 42,
    })
  })
  ...
  it.concurrent('rejects invalid preview version ids before building labels', () => {
    ...
    expect(() => buildPreviewSubdomain('com.example', Number.MAX_SAFE_INTEGER + 1)).toThrow(`Invalid preview version id: ${Number.MAX_SAFE_INTEGER + 1}`)
  })
  ...
  it.concurrent('rejects invalid preview channel ids before building labels', () => {
    ...
    expect(() => buildChannelPreviewSubdomain('com.example', Number.MAX_SAFE_INTEGER + 1)).toThrow(`Invalid preview channel id: ${Number.MAX_SAFE_INTEGER + 1}`)
  })

  it.concurrent('rejects preview hostnames whose version id exceeds the safe integer range', () => {
    expect(parsePreviewHostname('9007199254740992-com-0example.preview.codepushgo.app')).toBeNull()
  })

  it.concurrent('rejects preview hostnames whose channel id is not a positive safe integer', () => {
    expect(parsePreviewHostname('c0-com-0example.preview.codepushgo.app')).toBeNull()
    expect(parsePreviewHostname('c9007199254740992-com-0example.preview.codepushgo.app')).toBeNull()
  })
})
