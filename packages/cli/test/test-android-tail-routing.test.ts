...
describe('[Capgo parity] test-android-tail-routing', () => {
  it('keeps Android native build flow explicitly disabled for the RN MVP', () => {
    ...
    expect(nativeFlowFallback('android', 'test-android-tail-routing')).toMatchObject({ status: 'disabled', platform: 'android', flow: 'test-android-tail-routing' })
    expect(() => assertNativeBuildDisabled('android', 'test-android-tail-routing')).toThrow(/out of scope/)
  })
})
