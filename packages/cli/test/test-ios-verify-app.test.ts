...
describe('[Capgo parity] test-ios-verify-app', () => {
  it('keeps ios native build/store flow explicitly disabled for the RN MVP', () => {
    ...
    expect(nativeFlowFallback('ios', 'test-ios-verify-app')).toMatchObject({ status: 'disabled', platform: 'ios', flow: 'test-ios-verify-app' })
    expect(() => assertNativeBuildDisabled('ios', 'test-ios-verify-app')).toThrow(/out of scope/)
  })
})
