...
describe('[Capgo parity] test-android-play', () => {
  it('keeps Android native build flow explicitly disabled for the RN MVP', () => {
    ...
    expect(nativeFlowFallback('android', 'test-android-play')).toMatchObject({ status: 'disabled', platform: 'android', flow: 'test-android-play' })
    expect(() => assertNativeBuildDisabled('android', 'test-android-play')).toThrow(/out of scope/)
  })
})
