...
describe('[Capgo parity] test-ios-import-discovery', () => {
  it('keeps ios native build/store flow explicitly disabled for the RN MVP', () => {
    ...
    expect(nativeFlowFallback('ios', 'test-ios-import-discovery')).toMatchObject({ status: 'disabled', platform: 'ios', flow: 'test-ios-import-discovery' })
    expect(() => assertNativeBuildDisabled('ios', 'test-ios-import-discovery')).toThrow(/out of scope/)
  })
})
