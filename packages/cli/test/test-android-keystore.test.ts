...
describe('[Capgo parity] test-android-keystore', () => {
  it('keeps Android native build flow explicitly disabled for the RN MVP', () => {
    ...
    expect(nativeFlowFallback('android', 'test-android-keystore')).toMatchObject({ status: 'disabled', platform: 'android', flow: 'test-android-keystore' })
    expect(() => assertNativeBuildDisabled('android', 'test-android-keystore')).toThrow(/out of scope/)
  })
})
