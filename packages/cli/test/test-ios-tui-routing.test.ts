...
describe('[Capgo parity] test-ios-tui-routing', () => {
  it('keeps ios native build/store flow explicitly disabled for the RN MVP', () => {
    ...
    expect(nativeFlowFallback('ios', 'test-ios-tui-routing')).toMatchObject({ status: 'disabled', platform: 'ios', flow: 'test-ios-tui-routing' })
    expect(() => assertNativeBuildDisabled('ios', 'test-ios-tui-routing')).toThrow(/out of scope/)
  })
})
