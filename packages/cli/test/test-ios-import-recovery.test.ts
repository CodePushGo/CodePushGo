...
describe('[Capgo parity] test-ios-import-recovery', () => {
  it('keeps ios native build/store flow explicitly disabled for the RN MVP', () => {
    ...
    expect(nativeFlowFallback('ios', 'test-ios-import-recovery')).toMatchObject({ status: 'disabled', platform: 'ios', flow: 'test-ios-import-recovery' })
    expect(() => assertNativeBuildDisabled('ios', 'test-ios-import-recovery')).toThrow(/out of scope/)
  })
})
