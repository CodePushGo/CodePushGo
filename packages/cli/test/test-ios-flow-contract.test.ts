...
describe('[Capgo parity] test-ios-flow-contract', () => {
  it('keeps ios native build/store flow explicitly disabled for the RN MVP', () => {
    ...
    expect(nativeFlowFallback('ios', 'test-ios-flow-contract')).toMatchObject({ status: 'disabled', platform: 'ios', flow: 'test-ios-flow-contract' })
    expect(() => assertNativeBuildDisabled('ios', 'test-ios-flow-contract')).toThrow(/out of scope/)
  })
})
