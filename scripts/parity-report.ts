...
const semanticChecks: SemanticCheck[] = [
  ...
  semanticCheck('Capgo confirm-signup secure redirect route present', dashboardRoute.includes("'/confirm-signup'") && dashboardApp.includes('ConfirmSignupView') && dashboardConfirmationRedirect.includes('resolveConfirmationRedirect') && dashboardConfirmationRedirect.includes('supabaseUrl') && dashboardConfirmationRedirect.includes('protocol !== \'https:\''), 'dashboard routes /confirm-signup and only forwards confirmation URLs to console or Supabase hosts'),
  semanticCheck('Capgo resend-email auth flow route present', dashboardRoute.includes("'/resend_email'") && dashboardApp.includes('ResendEmailView') && dashboardRegistration.includes('resendSignupEmail') && dashboardRegistration.includes("type: 'signup'"), 'dashboard routes /resend_email and calls Supabase signup resend'),
  ...
  semanticCheck('Capgo SSO callback auth flow route present', dashboardRoute.includes("'/sso-callback'") && dashboardApp.includes('SsoCallbackView') && dashboardSsoCallback.includes('completeSsoCallback') && dashboardSsoCallback.includes('setSession') && dashboardSsoCallback.includes('exchangeCodeForSession') && dashboardSsoCallback.includes('validateRedirectPath') && dashboardSsoCallback.includes('clearAuthParamsFromUrl'), 'dashboard routes /sso-callback, exchanges Supabase SSO tokens/codes, clears tokens, and rejects unsafe redirects'),
