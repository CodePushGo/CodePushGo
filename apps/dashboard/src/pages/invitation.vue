...
const invitationTitle = computed(() => inviteRow.value ? `${t('welcome-to')} CodePushGo` : t('accept-invitation'))
...
function joinCodePushGo() {
  window.location.href = 'https://codepushgo.com/register/'
}

// Open ToS and Privacy Policy in new tabs
function openTos() {
  window.open('https://codepushgo.com/tos/', '_blank')
}

function openPrivacy() {
  window.open('https://codepushgo.com/privacy/', '_blank')
}
</script>
...
<template>
  ...
  >
    ...
    <div v-else class="space-y-5 text-center text-slate-500 dark:text-slate-300">
      ...
      <button :class="authSecondaryButtonClass" @click="joinCodePushGo">
        {{ t('join-capgo') }}
      </button>
    </div>
