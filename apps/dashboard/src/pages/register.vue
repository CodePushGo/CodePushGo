...
if (window.location.host === 'console.codepushgo.com') {
  // do not allow to register on webapp on production
  window.location.href = 'https://codepushgo.com/register/'
}
...
async function submit(form: { first_name: string, last_name: string, password: string, email: string }) {
  ...
  const { data: user, error } = await supabase.auth.signUp(
    ...
    // http://localhost:5173/login,http://localhost:5173/forgot_password?step=2,https://codepushgo.com/login,https://codepushgo.com/forgot_password?step=2,https://codepushgo.com/onboarding/first_password,https://development.codepushgo.com/login,https://development.codepushgo.com/forgot_password?step=2
  )
