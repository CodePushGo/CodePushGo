...
function previewPayloadUrlFromChannelLink(previewLink: Extract<PreviewDeepLink, { type: 'channel' }>) {
  ...
  try {
    ...
    return `https://${subdomain}.preview.codepushgo.com${PREVIEW_PAYLOAD_PATH}`
  }
  catch {
    return ''
  }
...
async function handleBarcodeScan(scannedValue: string) {
  ...
  if (!isHttpUrl(value)) {
    ...
    errorMessage.value = 'This QR code is not a CodePushGo preview link or an HTTPS bundle URL.'
    manualUrl.value = value
...
async function downloadUpdate(updateUrl: string) {
  ...
  if (!isHttpUrl(updateUrl)) {
    ...
    errorMessage.value = 'This is not a downloadable bundle URL. Use a CodePushGo preview QR code or an HTTPS bundle URL.'
        ...
        >
          <div class="rounded-2xl border border-white/10 bg-black/25 p-4 backdrop-blur-sm">
            ...
            <input
              ...
              inputmode="url"
              autocomplete="off"
              placeholder="https://preview.codepushgo.com/..."
