...
async function downloadNow() {
  ...
  if (version.value.session_key) {
    ...
    const command = `npx @codepushgo/cli@latest bundle decrypt ${localPath}  ${version.value.session_key} --key ./.capgo_key`
      ...
      <div class="mt-4">
        <div class="p-3 bg-gray-100 rounded-lg dark:bg-gray-800">
          <code class="text-sm break-all">
            npx @codepushgo/cli@latest bundle decrypt ./{{ version?.r2_path?.replace('/', '_') }} {{ version?.session_key }} --key ./.capgo_key
          </code>
        </div>
      </div>
    </Teleport>
