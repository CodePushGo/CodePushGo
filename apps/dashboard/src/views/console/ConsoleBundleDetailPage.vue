...
const bundle = computed(() => releases.value.find(release => String((release as typeof release & { id?: number | string }).id || '') === decodedBundleKey.value) || releases.value.find(release => release.platform === bundlePlatform.value && release.channel === bundleChannel.value && release.version === decodedBundle.value) || releases.value.find(release => release.version === decodedBundle.value))
