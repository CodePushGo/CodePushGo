...
describe('[Capgo parity] dashboard date helpers', () => {
  ...
  it.concurrent('formats month buckets with localized month names', () => {
    ...
    const expected = new Intl.DateTimeFormat(Intl.DateTimeFormat().resolvedOptions().locale || 'en', { month: 'short', year: 'numeric' }).format(date)
