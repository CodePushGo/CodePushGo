...
import { getErrorMessage, isKnownCrawlerNoiseErrorMessage, isStaleAssetErrorMessage, shouldSuppressPostHogExceptionEvent } from './staleAssetErrors'

describe('[Capgo parity] stale asset error helpers', () => {
  ...
  it('suppresses only stale asset exception events in PostHog', () => {
    expect(shouldSuppressPostHogExceptionEvent({
      event: '$exception',
      properties: { $exception_list: [{ value: 'Failed to fetch dynamically imported module: https://console.capgo.app/assets/dashboard-rYp22gdI.js' }] },
    })).toBe(true)
    expect(shouldSuppressPostHogExceptionEvent({
      event: '$exception',
      properties: { $exception_values: ['Unable to preload CSS for /assets/main-C3MIONxo.css'] },
    })).toBe(true)
    expect(shouldSuppressPostHogExceptionEvent({
      event: '$exception',
      properties: { $exception_list: [{ value: 'ResizeObserver loop completed with undelivered notifications.' }] },
    })).toBe(false)
    expect(shouldSuppressPostHogExceptionEvent({
      event: '$exception',
      properties: { $exception_list: [{ value: 'Object Not Found Matching Id:3, MethodName:update, ParamCount:4' }] },
    })).toBe(true)
  })
})
