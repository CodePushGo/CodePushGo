...
function isCodePushGoConsoleHost(hostname: string) {
  return hostname === 'console.codepushgo.com' || /^console\.(?:dev|preprod|staging)\.codepushgo\.com$/.test(hostname)
}
...
function handleDeepLink(router: Router, rawUrl: string) {
  ...
  if (url.protocol === 'https:' && isCodePushGoConsoleHost(url.hostname))
