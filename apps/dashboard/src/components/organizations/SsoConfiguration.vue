...
<template>
  ...
  >
    ...
    <div class="p-3 mb-3 space-y-2 font-mono text-sm bg-white border border-blue-200 rounded dark:bg-gray-800 dark:border-blue-700">
      ...
      <div class="flex items-center justify-between gap-2">
        <p class="text-slate-600 dark:text-slate-400">
          {{ t('sso-dns-record-name') }}: <span class="font-semibold text-slate-800 dark:text-white">_codepushgo-sso.{{ pendingVerificationProvider.domain }}</span>
        </p>
        <button
          class="d-btn d-btn-ghost d-btn-xs"
          :title="t('sso-copy')"
          @click="copyToClipboard(`_codepushgo-sso.${pendingVerificationProvider.domain}`, t('sso-dns-record-name'))"
        >
          <IconCopy class="w-4 h-4" />
        </button>
      </div>
