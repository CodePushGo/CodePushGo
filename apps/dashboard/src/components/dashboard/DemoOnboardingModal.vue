...
const CODEPUSHGO_CLI_COMMAND = 'bunx @codepushgo/cli@latest'
...
const stepContentMap: Record<DemoStep, StepContent> = {
  1: {
    ...
    description: 'Type your app name to see how the CodePushGo flow works.',
...
const updateChoices: UpdateChoice[] = [
  {
    ...
    command: `${CODEPUSHGO_CLI_COMMAND} bundle upload <appId> --path ./dist --channel production --comment "Confetti demo payload"`,
  ...
  },
  {
    ...
    command: `${CODEPUSHGO_CLI_COMMAND} bundle upload <appId> --path ./dist --channel production --comment "Background color demo payload"`,
    terminalLine: 'Preparing release bundle for distribution',
  },
  {
    id: 'title',
    label: 'Change the title',
    result: 'Welcome back to CodePushGo',
    command: `${CODEPUSHGO_CLI_COMMAND} bundle upload <appId> --path ./dist --channel production --comment "Title change demo payload"`,
...
const terminalLiveMessage = computed(() => {
  if (isCreateStep.value)
    return isCreatingApp.value ? 'Creating app in CodePushGo...' : ''
  if (isUploadStep.value)
    return isUploading.value ? 'Uploading to CodePushGo...' : ''
  return ''
})
...
function getAddAppCommand(appId: string, name: string) {
  return `${CODEPUSHGO_CLI_COMMAND} app add ${appId} --name "${name}"`
}

function getUploadCommand(appId: string) {
  return `${CODEPUSHGO_CLI_COMMAND} bundle upload ${appId} --path ./dist --channel production`
}
...
function selectApp(choice: AppChoice) {
  ...
  queueTerminalLines(createLines, [
    ...
    { line: `Adding ${appId} to CodePushGo`, delay: 900 },
    { line: `✔ App ${appId} added to CodePushGo`, delay: 1500 },
    { line: `Next step: upload a bundle with "npx @codepushgo/cli bundle upload"`, delay: 1800 },
  ])
...
function triggerUpload(action: UpdateChoice) {
  ...
  queueTerminalLines(uploadLines, [
    ...
    { line: '  - sending update to CodePushGo edge', delay: 2550 },
            ...
            <div class="flex-1 space-y-4">
              <div v-if="isCreateStep" class="flex flex-col gap-3">
                ...
                <p v-if="isCreatingApp" class="text-xs text-base-content/60">
                  Running <span class="font-mono text-emerald-600">{{ CODEPUSHGO_CLI_COMMAND }} app add</span> ...
                </p>
              </div>
                              ...
                              <div class="flex flex-col items-center justify-center flex-1 p-4 text-center">
                                ...
                                <template v-else>
                                  ...
                                  <p class="text-sm opacity-70 max-w-[180px]">
                                    {{ hasOpenedUpdatedApp ? 'Update successfully applied via CodePushGo!' : 'This is the initial version of your app.' }}
                                  </p>
                                </template>
                              </div>
