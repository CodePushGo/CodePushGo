 ...
 *
 * A self-contained, 5-slide animated presentation for "CodePushGo Builder" (native
...
// real, scannable QR → the public CodePushGo Builder page
let qrDataUrl = ''
try {
  qrDataUrl = svgToDataUrl(toSvg('https://codepushgo.com/native-build/', { margin: 1, moduleSize: 4, foreground: '#0a0d14', background: '#e8f3ff' }))
}
catch {
  qrDataUrl = ''
}
...
function makeScript(p: 'ios' | 'android'): TermStep[] {
  const cmd = `<span class="prompt">$ </span><span class="cmd">npx @codepushgo/cli@latest build request</span> <span class="flag">--platform</span> <span class="val">${p}</span>`
  if (p === 'android') {
    return [
              ...
              <div class="bp-left bp-left--term">
                <div class="bp-terminal">
                  <div class="bp-term-bar">
                    ...
                    <span class="bp-term-title">codepushgo build request --platform {{ termPlatform }}</span>
