...
const tabs = computed<Tab[]>(() => {
  const baseTabs: Tab[] = [
    ...
    },
    {
      ...
      icon: IconDoc,
      key: '#',
      onClick: () => window.open('https://codepushgo.com/docs', '_blank', 'noopener,noreferrer'),
      redirect: true,
    },
    {
      label: 'discord',
      icon: IconDiscord,
      key: '#',
      onClick: () => window.open('https://discord.gg/codepushgo', '_blank', 'noopener,noreferrer'),
      redirect: true,
    },
  ]
    ...
    >
      ...
      <div class="flex justify-between px-3 py-4 border-b lg:py-6 lg:px-6 lg:border-b border-slate-800 shrink-0 lg:border-slate-700">
        <router-link
          ...
          to="/apps"
          aria-label="CodePushGo - Go to dashboard"
        >
          <img src="/capgo.webp" alt="CodePushGo logo" class="w-8 h-8">
          <span class="text-xl font-semibold truncate transition duration-150 hover:text-white font-prompt text-slate-200 lg:text-slate-200 lg:hover:text-white">CodePushGo</span>
        </router-link>
      </div>
