...
const browserPreviewHelp = computed(() => {
  if (props.browserPreviewUnavailableReason === 'missing-manifest') {
    return {
      ...
      command: 'npx @codepushgo/cli@latest bundle upload --delta',
    }
  }
...
const previewUrl = computed<string | null>(() => {
  try {
    ...
    // Extract base domain from current host, default to codepushgo.com for localhost
    // Preserve environment segments (e.g., 'dev' in console.dev.codepushgo.com)
    const hostname = globalThis.location.hostname
    let baseDomain = 'codepushgo.com'
